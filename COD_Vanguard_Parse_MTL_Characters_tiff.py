"""
Name-US:COD Vanguard Characters Import .mtl (tiff) c4d...
...
MTL_KEYWORDS_MAP = {
                     ...
                     'unk_semantic_0x0' : c4d.MATERIAL_COLOR_SHADER,
                     'unk_semantic_0xC0' : c4d.MATERIAL_COLOR_SHADER,
                     'specColorMap' :c4d.REFLECTION_LAYER_LAYER_DATA + c4d.REFLECTION_LAYER_LAYER_SIZE*4 + c4d.REFLECTION_LAYER_COLOR_TEXTURE,
                     'unk_semantic_0x8' : c4d.MATERIAL_NORMAL_SHADER,
                     'unk_semantic_0xC1' : c4d.MATERIAL_NORMAL_SHADER,
                     'aoMap' : c4d.MATERIAL_DIFFUSION_SHADER,
                     'unk_semantic_0x10' : c4d.MATERIAL_LUMINANCE_SHADER,
                     ...
                     'unk_semantic_0x18' : c4d.MATERIAL_ALPHA_SHADER
                   }
...
def InsertTexture(fBase, fName, matName, targetId, targetDoc, flipNormalY, enableAlpha, textureType,textureChar):
    DebugPrint('  -> Inserting "' + fName + '" into channel "' + MTL_KEYWORDS_NAMES[targetId] + '" of material "' + matName + '".')
    mat = targetDoc.SearchMaterial(matName)
    fNames = fName.split(textureChar)
    fSlash = '/'

    if textureChar == 'a' :

        fPath = fPath = str(fBase) + str(fSlash) + str(fName)
        ...
        #fPath = str(fBase) + str(fSlash) + str(fNames[0])+  textureType + ".tiff"
    ...
    for ln in fl:
        ...
        # Check for maps
            ...
            if matName == '':
                ...
                if words[0] == 'unk_semantic_0xC0' or 'unk_semantic_0x0':

                     DebugPrint('ENABLE ALPHA')
                     enableAlpha = 1
                     ChannelArr = ['unk_semantic_0x0' ,'specColorMap' ]
                     textureType = ['c' ,'s' ]
                     textureChar = '&'
                     DebugPrint('  Found map ' + MTL_KEYWORDS_NAMES[MTL_KEYWORDS_MAP[words[0]]] + ': ' + words[1])
                     j = 0;
                     while j<2:
                            InsertTexture(basePath, mapName+'.tiff', matName , MTL_KEYWORDS_MAP[ChannelArr[j]], targetDoc, normalFlipY, enableAlpha, textureType[j], textureChar)
                            enableAlpha = 0
                            j += 1

                if words[0] == 'unk_semantic_0xC1' or 'unk_semantic_0x8' :

                     ChannelArr = ['unk_semantic_0x8' ,'glossMap','aoMap']
                     textureType = ['n' ,'g','o' ]
                     textureChar = '&'
                     normalFlipY = 1
                     j = 0;
                ...
                if words[0] == 'unk_semantic_0x10':


                    ChannelArr = ['unk_semantic_0x10']
                    textureType = ['']
                    textureChar = 'e'
                    DebugPrint('  Found map ' + MTL_KEYWORDS_NAMES[MTL_KEYWORDS_MAP[words[0]]] + ': ' + words[1])
                    InsertTexture(basePath, mapName+'.tiff', matName , MTL_KEYWORDS_MAP[ChannelArr[0]], targetDoc, normalFlipY, enableAlpha, textureType[0],textureChar)
                    
                if words[0] == 'unk_semantic_0x18':


                   ChannelArr = ['unk_semantic_0x18']
                   textureType = ['']
                   textureChar = 'a'
                   DebugPrint('  Found map ' + MTL_KEYWORDS_NAMES[MTL_KEYWORDS_MAP[words[0]]] + ': ' + words[1])
                   InsertTexture(basePath, mapName+'.tiff', matName , MTL_KEYWORDS_MAP[ChannelArr[0]], targetDoc, normalFlipY, enableAlpha, textureType[0],textureChar)
